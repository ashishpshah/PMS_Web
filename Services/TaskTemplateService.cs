...
namespace TaskManagement.Services
{
    ...
    public class TaskTemplateService : ITaskTemplateService
    {
        ...
        public async Task<ApiResponse<TaskTemplateDto>> UpdateAsync(int id, SaveTaskTemplateDto dto, int userId)
        {
            ...
            {
                if (!dto.Items.Any())
                    return Fail<TaskTemplateDto>("Template has no task items configured.");
                if (dto.Items.Any(i => i.DefaultAssigneeId == null))
                    return Fail<TaskTemplateDto>("Every task item must have an assignee before activating the template.");
            }
        ...
        public async Task<ApiResponse<bool>> SetActiveAsync(int id, bool isActive)
        {
            ...
            {
                if (!t.Items.Any())
                    return Fail<bool>("Template has no task items configured.");
                if (t.Items.Any(i => i.DefaultAssigneeId == null))
                    return Fail<bool>("Every task item must have an assignee before activating the template.");
            }
