...
import { ReasonTag, BLOCK_REASON_TAGS, STATUS_BADGE_VARIANT, STATUS_LABELS, DashboardEffort, DashboardStats } from '../types';
...
export default function Dashboard() {
  const { projects, tasks, users, assignableUsers, loading, reassignTask } = useData();
  ...
  // ── Dashboard stats (all 7 statuses + Section 10 metrics) ─────────────────
  ...
  // Blocked tasks counted by status (not the legacy isBlocked flag)
  ...
  return (
    <PageTransition>
      ...
      <div className="space-y-6">
        ...
        {dashboardStats && (() => {
          const STATUS_CFG: { key: string; label: string; dot: string; bar: string; text: string }[] = [
                    ...
                    {STATUS_CFG.map(s => {
                      ...
                      return (
                        <div key={s.key} className="flex items-center gap-3">
                          <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 w-20 shrink-0">{s.label}</span>
