...
/** Upload a file using multipart/form-data, with the same auth/refresh/error handling as apiRequest. */
export async function apiUpload<T>(endpoint: string, file: File, fieldName = 'file', opts?: ApiRequestOpts): Promise<T> {
  const url = `${getApiUrl()}${endpoint}`;
  const token = getToken();
  if (!opts?.silent) beginRequest();
  try {
